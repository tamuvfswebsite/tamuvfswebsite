...
module ResumeFiltering
  ...
  def fetch_filtered_resumes
    ...
    # Exclude resumes from sponsor users
    Resumes::IndexQuery.call(build_filter_params).joins(:user)
                       .where.not(users: { role: 'sponsor' }).page(params[:page]).per(per_page)
  end
  ...
  def load_filter_options
    ...
    @organizational_roles = OrganizationalRole.order(:name)
